...
if __name__ == "__main__":
    ...
    parameter = parameters[ihydro]

    activity = parameter['activity']
    iexcs = [int(el.replace("__exchange_", ""))for el in list(parameter['exchanges'])]
    exchanges = [activity['exchanges'][i] for i in iexcs if activity['exchanges'][i]['uncertainty type'] > 1]
    ...
    # exc_names = [exc[:40] for exc in exc_names_temp]
    # exc_names = []
    # k = 0
    ...
    #         try:
    #             el = str(exc_names_temp[k])
    #         except IndexError:
    #             el = str()
    #         k += 1
    #         exc_names_row.append(el)
    #     exc_names.append(exc_names_row)
    ...
    fig.update_layout(
        ...
        margin=dict(t=150, b=10, l=10, r=0),
        paper_bgcolor="rgba(255,255,255,1)",
        plot_bgcolor="rgba(255,255,255,1)",
    )
    fig.write_image(write_figs / f"psamples_{activity['name'][:40]}_{activity['location'][:3]}.eps")
